...
(deftest linear-least-squares-line-fitting-test
  (with-instrument `curve-fitting/linear-least-squares-line-fitting
  ...
  (with-instrument (st/instrumentable-syms)
    (let [lls-map (curve-fitting/linear-least-squares-line-fitting
                    {::curve-fitting/x-vals   xs
                     ...
                     ::curve-fitting/basis-fn (fn [x]
                                                [1.0 x (m/cube x)])})]
      (is
        (data-approx= [-374.1841730714473 198.65888604231853 -2.7220798937280524]
    ...
    (let [lls-map-x (curve-fitting/linear-least-squares-line-fitting
                      {::curve-fitting/x-vals   (conj xs 6.0) ;can have duplicate x-vals
                       ...
                       ::curve-fitting/basis-fn (fn [x]
                                                  [1.0 x (m/cube x)])})]
      (is
        (data-approx= [-287.27029929684994 162.806101134645 -2.3851099982080766]
    ...
    (let [lls-map2 (curve-fitting/linear-least-squares-line-fitting
                     {::curve-fitting/x-vals   xs
                      ...
                      ::curve-fitting/basis-fn (fn [x]
                                                 [1.0 (m/sq x) (m/cube x)])})]
      (is (data-approx= [-147.53975088364837 49.710219695486124
                         -6.462473288353114]
    ...
    (let [ch1-4 (curve-fitting/linear-least-squares-line-fitting
      ...
      (is (data-approx= [-1377.7020833331776 1360.6833333331895
                         -267.49999999997624 24.316666666665014
                         -0.7979166666666255]
    ...
    (let [ch1-3 (curve-fitting/linear-least-squares-line-fitting
      ...
      (is (data-approx=
            [1125.4528301886708 -953.7825696316196 136.6630727762796
             -5.672955974842742]
    ...
    (let [ch2-3 (curve-fitting/linear-least-squares-line-fitting
      ...
      (is (data-approx=
            [1057.1212938004992 -474.05480682837816 68.3315363881387
             -2.8364779874213335]
    ...
    (let [p1-4 (curve-fitting/linear-least-squares-line-fitting
      ...
      (is (data-approx=
            [116.73530443936708 -108.88714394337724 34.8383049657826
             -3.0789904661636016]
...
(deftest b-spline-line-fitting-test
  (with-instrument `curve-fitting/b-spline-line-fitting
...
(deftest linear-least-squares-curve-fitting-test
  (with-instrument `curve-fitting/linear-least-squares-curve-fitting
  ...
  (with-instrument (st/instrumentable-syms)
    ...
    (let [lls-curve (curve-fitting/linear-least-squares-curve-fitting
      ...
      (is (data-approx=
            [3576.2055568677993 -1644.3254459626135 107.97588652482267
             15.591742575196873]
    ...
    (let [poly-20 (curve-fitting/linear-least-squares-curve-fitting
      ...
      (is (data-approx=
            [-4570.710184016991 -348.5196618375316 7126.080272273072
             29.4624197555226 -239.66381568606565 -1377.9959457490452
             -0.37825629375083264 -5.214477757191738 56.624414816925864
             114.13616877081664 0.0045612061972588325 -0.03113641892213491
             0.6647388129750418 -5.569874056570459 -3.345550489699451
             -1.7560473159920388E-5 2.0814908582451206E-5 0.001121204887739387
             -0.024356456660173074 0.18373754643701698]
