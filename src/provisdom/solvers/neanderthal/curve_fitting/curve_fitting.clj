(ns provisdom.solvers.neanderthal.curve-fitting.curve-fitting
  (:require
...
;;;LINE FITTING
(s/def ::f-vals (vector/vector-finite-spec {:min-count 2}))
(s/def ::x-vals (vector/vector-finite-spec {:min-count 4}))
...
(s/def ::x-vals-with-f-vals
  (s/with-gen
    ...
    #(gen/bind
       ...
       (fn [x]
         (gen/bind
           (gen/vector (s/gen ::m/num) (count x))
           (fn [f-v]
             (gen/return {::x-vals x
                          ::f-vals f-v})))))))
...
(s/def ::distinct-x-vals
  (vector/vector-finite-spec {:distinct? true :min-count 2}))
...
(s/fdef linear-least-squares-curve-fitting
  ...
  :ret (s/or :sol (s/keys :req [::curve-fitting-fn
                                ::curve-fitting-weights])
         :anomaly ::anomalies/anomaly))
