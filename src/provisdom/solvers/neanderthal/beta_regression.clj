...
(defn gradient-old
  ...
  (let [beta (vec beta)
        ...
        y* (mapv y* y)
        T (mapv #(* (special-fns/logistic %) (m/one- (special-fns/logistic %)))
            eta)
        Ty*-mu* (mapv (fn [t mu* y*]
                        (* phi t (- y* mu*)))
                  T mu* y*)
        u_beta (mx/get-column
                 (mx/mx* (mx/transpose x-mx) (mx/column-matrix Ty*-mu*))
                 0)
        u_phi (reduce +
                (map
                  (fn [mu y]
...
(defn solve
  ...
  ([x-mx
    y
    beta0
    {::keys [old? max-iter max-evaluations abs-accu]
     :or    {abs-accu 1e-8 max-iter 1000 max-evaluations 1000}}]
   (let [n (count y)
         X (nat/dge x-mx)
         yv (nat/dv y)
     ...
     (descent (comp - (partial objective x-mx y))
       #(mapv - (gradient-old x-mx y %))
       beta0
       1e-3
       0.00001
       1000)
     #_(nlp/unbounded-nonlinear-programming
         {::nlp/objective  (partial objective x-mx y)
     ...
     #_(nlls/nonlinear-least-squares
         {::nlls/constraints-fn         (if old? grad1-old grad2)
